    ...
    pub fn check_flags(protocol: Protocol, available_flags: u32, required_flags: u32) -> bool {
        match protocol {
            ...
            Protocol::JobDeclarationProtocol => {
                ...
                // - False if `full_template_mining_required_self` is true and
                //   `allow_full_template_mining_passed` is false.
                // - True otherwise.
                //
                ...
                let allow_full_template_mining_passed = (required >> 31) & 1 > 0;
                let full_template_mining_required_self = (available >> 31) & 1 > 0;

                match (
                    full_template_mining_required_self,
                    allow_full_template_mining_passed,
                ) {
                    (true, true) => true,
                    (true, false) => false,
                    (false, true) => true,
