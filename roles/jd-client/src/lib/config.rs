...
///    (TP) to receive job templates. The pool connection details are specified in the `upstreams`
///    field, and the TP connection details are in `tp_address`.
...
pub struct JobDeclaratorClientConfig {
    ...
    // A flag indicating whether the JDC should withhold shares.
    withhold: bool,
    // The public key used by this JDC for noise.
    authority_public_key: Secp256k1PublicKey,
    /// The secret key used by this JDC for noise.
...
impl JobDeclaratorClientConfig {
    ...
    ///
    /// This constructor allows creating a `JobDeclaratorClientConfig` programmatically
    /// using separate configuration structs for different aspects (protocol, pool, TP).
