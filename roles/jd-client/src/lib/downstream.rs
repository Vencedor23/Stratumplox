...
pub struct DownstreamMiningNode {
    ...
    // or other components that handle solution submission.
    solution_sender: Sender<SubmitSolution<'static>>,
    // Flag indicating whether block rewards should be withheld (configured by the JDC).
    withhold: bool,
    ...
    async fn match_send_to(
        ...
        match next_message_to_send {
            ...
            Ok(SendTo::RelayNewMessage(Mining::SubmitSharesExtended(mut share))) => {
                // This case is for pooled mining, where the JDC forwards a potentially modified
                // share.
